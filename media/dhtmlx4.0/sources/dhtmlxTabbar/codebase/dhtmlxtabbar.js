...
dhtmlXTabBar.prototype._fixTabsOfs = function(id) {
...
dhtmlXTabBar.prototype.getNumberOfTabs = function(mode) {
	// mode - set to tru for visible only (new)
	if (mode !== true) return this.tabsArea.childNodes[1].firstChild.childNodes.length;
	var p = this.tabsArea.childNodes[1].firstChild;
	var i = 0;
	for (var q=0; q<p.childNodes.length; q++) {
		var id = p.childNodes[q]._tabId;
		if (id != null && this.t[id].conf.visible) i++;
	}
	p = null;
	return i;
};
