...
dhtmlXRibbon.prototype._covertXmlNodeToObject = function(node){
	...
	if (node.childNodes.length) {		
		if (node.getAttribute('type') === "buttonCombo" 
		 || node.getAttribute('type') === "buttonSelect"  ) {
			answer.data = node;
...
dhtmlXRibbon.prototype._addItem = function(blockId, nextToId, itemId, data){
	...
	if (base) {
		...
		item = (this.items[itemData.type] && this.items[itemData.type].render)
			? this.items[itemData.type].render(base,itemData)
			: null;
	}
...
dhtmlXRibbon.prototype.items.buttonSegment = {
	...
	_getSelectedNeighbor : function(item, rb){
		var parent = rb._items[item.parentId],
			i,l, neighbor, answer = null;
...
//			}
		...
		dhtmlXRibbon.prototype.items.group.normalise(item);
...
dhtmlXRibbon.prototype.items.input = {
	render : function(cont, itemData){
		...
		cont.innerHTML = "<input type='text' class='dhxrb_input'>"
			+"<div class='dhxrb_label_button'>"+item.conf.text+"</div>";
...
dhtmlXRibbon.prototype.items.checkbox = {
	render : function (cont, itemData) {
		...
		cont.innerHTML = "<div class='dhxrb_checkbox'></div>"
			+"<div class='dhxrb_label_checkbox'>"+item.conf.text+"</div>";
...
dhtmlXRibbon.prototype.items.text = {
	render : function(cont, data){
		...
		cont.innerHTML = "<div class='dhxrb_item_text'>"
				+item.conf.text
			+"</div>";
...
dhtmlXRibbon.prototype.items.buttonCombo = {
	render : function(cont, dataItem){
		...
		item.base.innerHTML = "<div class='dhxrb_buttoncombo'></div>"
			+"<div class='dhxrb_label_button'>"
				+item.conf.text
			+"</div>";
		...
		item.combo.setSkin(comboConf.skin); // deprecate;
...
dhtmlXRibbon.prototype.items.slider = {
	render : function(cont, data){
		...
		item.base.innerHTML = "<center><div class='dhxrb_slider'></div></center>"
			+"<div class='dhxrb_label_button'>"
				+item.conf.text
			+"</div>";
...
dhtmlXRibbon.prototype._addNewLevel = function (block) {
	...
	if (block.conf.mode === 'rows') {
		...
		if (lastNode 
			&& /dhxrb_block_rows/i.test(lastNode.className) 
			&& (lastNode.childNodes.length < 3) ) {
...
dhtmlXRibbon.prototype._getContainerForSmallItem = function(parent){
	...
	if (parent.conf.mode === "rows") {
		...
		if (last_el 
			&& /dhxrb_3rows_block/i.test(last_el.className)
			&& (last_el.childNodes.length < 3) ) {
...
//		var lastNode = parent.base.lastChild;
//		if (lastNode && /dhxrb_separator_group/i.test(lastNode.className)) {
//			lastNode.parentNode.removeChild(lastNode);
//		}
		dhtmlXRibbon.prototype.items.group.normalise(parent);
	}
	...
	if (this.items[item.type]
		&& (typeof(this.items[item.type].remove) === "function") ) {
...
dhtmlXRibbon.prototype.hide = function(id){
	...
	if (this.items[item.type] 
		&& (typeof(this.items[item.type].hide) === "function")) {
...
dhtmlXRibbon.prototype.getValue = function(id){
	...
	if (this.items[item.type] 
		&& (typeof(this.items[item.type].getValue) === "function") ){
		return this.items[item.type].getValue(item);
	}
	
};

dhtmlXRibbon.prototype.setValue = function(id, value){
	var item = this._items[id];
	
	if (this.items[item.type] 
		&& typeof(this.items[item.type].setValue) === "function"){
		this.items[item.type].setValue(item, value);
	}
...
dhtmlXRibbon.prototype.check = function(id,callEvent){
	...
	if (item && !item.conf.checked
		&& typeof(this.items[item.type].check) === "function") {
...
dhtmlXRibbon.prototype.uncheck = function(id, callEvent){
	...
	if (item && item.conf.checked
		&& typeof(this.items[item.type].uncheck) === "function") {
...
dhtmlXRibbon.prototype.disable = function(id,activetab){
	...
	if (this.items[item.type]
		&& (typeof(this.items[item.type].disable) === "function") ) {
		if (this.items[item.type].disable(item) !== true) {
			return;
		}
	}
...
dhtmlXRibbon.prototype.enable = function(id,activetab){
	...
	if (this.items[item.type]
		&& (typeof(this.items[item.type].enable) === "function") ) {
...
dhtmlXRibbon.prototype.isEnabled = function(id){
	...
	if (this.items[item.type]
		&& typeof(this.items[item.type].isEnabled) === "function") {
...
dhtmlXRibbon.prototype.isVisible = function(id){
	...
	if (this.items[item.type]
		&& (typeof(this.items[item.type].isVisible) === "function") ) {
