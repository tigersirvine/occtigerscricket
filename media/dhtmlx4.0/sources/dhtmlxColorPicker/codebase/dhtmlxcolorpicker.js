...
dhtmlXColorPicker.prototype.show = function(node) {
	...
	if (this.activeNode) {
		is_show_memory = (this.activeNode.conf.customColors != null)? 
		this.activeNode.conf.customColors:
		this.conf.customColors;
	...
	if (this.base._dhx_remove) {
		...
		if (document.body.firstChild) document.body.insertBefore(this.base, document.body.firstChild);
...
dhtmlXColorPicker.prototype.setPosition = function(x,y) {
	...
	if (isNaN(cx)) {
	pos = (x.toLowerCase() == "left")? "left": (x.toLowerCase() == "bottom")? "bottom": null;
	}
...
dhtmlXColorPicker.prototype._refreshPosition = function(position) {
	...
	var topInp = dhx4.absTop(this.activeNode.node), leftInp = dhx4.absLeft(this.activeNode.node),
	sizeWindow = dhx4.screenDim(), heigthCP = this._globalNode.offsetHeight,
	top = 0;
	position = position || this.conf.position;
	
	switch (position) {
		case "bottom":
			this._globalNode.style.top = topInp+this.activeNode.node.offsetHeight+ this.conf.indent +"px";
			this._globalNode.style.left = leftInp + "px";
			break;
		default:
			this._globalNode.style.left = leftInp + this.activeNode.node.offsetWidth + this.conf.indent +"px";
			top = topInp;
			
			if (sizeWindow.bottom - (top + heigthCP) <= 0) {
				top = sizeWindow.bottom - (heigthCP + this.conf.indent);
			}
...
dhtmlXColorPicker.prototype._refreshColorValue = function() {
	this._controllerNodes.color.style.backgroundColor = "rgb("
	+[this.value.red, this.value.green, this.value.blue].join(", ")
	+")";
...
dhtmlXColorPicker.prototype.Memory = function(base) {
	...
	this.setValue = function(value, contr, callEvent) {
		...
		selected.domElement.style.backgroundColor = "rgb("
		+[value.red, value.green, value.blue].join(", ")
		+")";
	...
	this.clean = function() {
		...
		for (var i=0; i<l; i++) {
			...
			item.domElement.style.backgroundColor = "rgb("
			+[dfValue.red, dfValue.green, dfValue.blue].join(", ")
			+")";
		}
		this.unSelect();
		selected = this.controllers[0];
